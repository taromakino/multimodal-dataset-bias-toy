...
def make_standard_data(rng, input_dim, n_examples, u_sd, x_sd, y_sd):
    ...
    x0_noise = rng.multivariate_normal(mean=np.zeros(input_dim), cov=make_isotropic_cov(input_dim, x_sd), size=n_examples)
    x1_noise = rng.multivariate_normal(mean=np.zeros(input_dim), cov=make_isotropic_cov(input_dim, x_sd), size=n_examples)
...
def make_selection_biased_data(rng, input_dim, n_examples, u_sd, x_sd, y_sd, alpha):
    mlp = MLP(2, [64], 1)
    u_all, x_all, y_all = [], [], []
    count = 0
    while count < n_examples:
        u, x, y = make_standard_data(rng, input_dim, n_examples, u_sd, x_sd, y_sd)
        collider = mlp(torch.tensor(u)[:, None], torch.tensor(y)[:, None]).detach().numpy()
        collider = (collider - collider.mean()) / collider.std()
        prob = sigmoid(alpha * collider)
...
def make_data(seed, input_dim, n_examples, u_sd, x_sd, y_sd, alpha, is_normalizing, is_including_u, batch_size, n_workers):
    n_train, n_val, n_test = n_examples
    n_trainval = n_train + n_val
    rng = np.random.RandomState(seed)
    if alpha is None:
        u_trainval, x_trainval, y_trainval = make_standard_data(rng, input_dim, n_trainval, u_sd, x_sd, y_sd)
    else:
        u_trainval, x_trainval, y_trainval = make_selection_biased_data(rng, input_dim, n_trainval, u_sd, x_sd, y_sd, alpha)
    u_test, x_test, y_test = make_standard_data(np.random.RandomState(2 ** 32 - 1), input_dim, n_test, u_sd, x_sd, y_sd)
