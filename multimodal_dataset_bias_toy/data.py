...
def make_raw_data(rng, sample_size, input_dim, origin_offset, x_sd, temperature):
    u_dim = 2 * input_dim
    u = rng.multivariate_normal(mean=np.zeros(u_dim), cov=np.eye(u_dim), size=sample_size)
    eps_x0 = rng.multivariate_normal(mean=np.zeros(input_dim), cov=make_isotropic_cov(input_dim, x_sd), size=sample_size)
    eps_x1 = rng.multivariate_normal(mean=np.zeros(input_dim), cov=make_isotropic_cov(input_dim, x_sd), size=sample_size)
...
def make_data(seed, n_examples, input_dim, origin_offset, x_sd, temperature, is_normalize, is_include_u, batch_size, n_workers):
    rng = np.random.RandomState(seed)
    n_train, n_val, _ = n_examples
    u, x, y = make_raw_data(rng, sum(n_examples), input_dim, origin_offset, x_sd, temperature)
